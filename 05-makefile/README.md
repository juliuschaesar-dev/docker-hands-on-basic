...
## Exercise 1 — Baca Makefile

1. Buka `Makefile`, identifikasi komponennya: **variable** (`SCRIPT`, `OUTPUT`), **target**, **prerequisite**, dan **recipe** (baris perintah).
2. Perhatikan target `all: help` — target apa yang dijalankan kalau kamu ketik `make` saja tanpa argumen?
3. Perhatikan `@` di depan `@echo` — coba jelaskan bedanya dengan baris tanpa `@` (`bash $(SCRIPT)`) saat dijalankan.

## Exercise 2 — Jalankan target
...
## Exercise 3 — Gabungkan dengan Docker
